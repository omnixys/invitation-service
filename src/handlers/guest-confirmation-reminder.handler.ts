...
export class GuestConfirmationReminderHandler {
  ...
  async remind(payload: { invitationId: string; actorId?: string }): Promise<void> {
    const { invitationId, actorId } = payload;
    ...
    try {
      ...
      if (invitation.eventEndsAt && invitation.eventEndsAt.getTime() <= Date.now()) {
        this.logger.debug('Confirmation reminder skipped, event already ended: %o', {
          invitationId,
        });
        return;
      }
      ...
      if (
        (invitation.confirmationResendCount ?? 0) >= env.GUEST_REMINDER_MAX_RESENDS
      ) {
        this.logger.warn(
          'Confirmation reminder skipped, max resends reached: %o',
          { invitationId, resendCount: invitation.confirmationResendCount },
        );
        return;
      }
